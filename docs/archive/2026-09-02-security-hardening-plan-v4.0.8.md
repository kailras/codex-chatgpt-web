> **아카이브** — 이 문서는 v4.0.8 보안 강화 계획의 완료본입니다. 2026-09-16 upstream v5.0.7 병합(`434b836`) 이후
> 재검증에서 아래 조치가 모두 코드에 유지되고 있음을 확인했습니다(`tests/server-security.test.ts`,
> `tests/tunnel.test.ts`, `launcher/tests/packaging-contract.test.cjs` 통과). 현재 계획은 저장소 루트의 `PLAN.md`를 보십시오.
