...
- **한국어 완벽 지원**: 런처 UI, 진단(Doctor) 안내, 시스템 트레이, 사용자 문서 전체가 한국어로 제공됩니다.
...
1. **ChatGPT 로그인**:
   - 런처 내장 브라우저 창에서 사용할 ChatGPT 계정으로 로그인합니다.
...
런처는 별도의 독립된 브라우저 프로필을 유지합니다. 외부 일반 브라우저에서 로그인하더라도 런처 내장 브라우저와는 분리되어 있으므로, 반드시 런처 내에서 로그인을 완료해 주세요.
